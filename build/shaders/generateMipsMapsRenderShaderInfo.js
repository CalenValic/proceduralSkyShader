...
const shaderName = "generateMipMapsRenderShader";
export const generateMipMapsRenderShaderInfo = {
    loadPipeline: () => {
    ...
    render: (commandEncoder, loadOp, properties) => {
        ...
        for (var textureID in textureManager.textures) {
            ...
            if (texture.texture.mipLevelCount > 1) {
                for (var baseMipLevel = 1; baseMipLevel < texture.texture.mipLevelCount; baseMipLevel++) {
                    for (var layer = 0; layer < texture.texture.depthOrArrayLayers; layer++) {
